# Atrial fibrillation detection installation guide
...
To move beyond notebook prototyping, all reusable code should go into the `afdetection/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `afdetection` folder and use the modules inside your notebooks :
...
```python
from afdetection.utils.paths import data_dir
data_dir()
```
